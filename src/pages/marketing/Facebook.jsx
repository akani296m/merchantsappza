...
export default function FacebookMarketing() {
    ...
    useEffect(() => {
        const fetchPixelId = async () => {
            ...
            if (merchant.facebook_pixel_id) {
                setPixelId(merchant.facebook_pixel_id);
                return;
            }
            ...
            try {
                ...
                if (!error && data?.facebook_pixel_id) {
                    setPixelId(data.facebook_pixel_id);
                }
            } catch (err) {
                console.error('Error fetching Facebook Pixel ID:', err);
            }
        };

        fetchPixelId();
    }, [merchant?.id, merchant?.facebook_pixel_id]);
    ...
    const handleSave = async () => {
        ...
        try {
            const { error } = await supabase
                .from('merchants')
                .update({ facebook_pixel_id: pixelId.trim() || null })
                .eq('id', merchant.id);
